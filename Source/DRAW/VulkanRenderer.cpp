...
#pragma endregion
	...
	void Update_VulkanRenderer(entt::registry& registry, entt::entity entity)
	{
		...
		if (registry.all_of< VulkanVertexBuffer, VulkanIndexBuffer>(entity))
		{
			...
			//creating a group of all GPUInstance and GeometryData in the registry
			auto ref = registry.group<GPUInstance>(entt::get< GeometryData>,entt::exclude<DoNotRender>); //could use entt::exclude<component3> to exclude specific types from the group
			//THIS IS WHY PLAYER AND ENEMY ARE BEING DRAWN TWICE. NOT SURE IF I NEED TO ADD SOMETHING HERE OR SOMEWHERE ELSE TO ADJUST. BECAUSE IM MAKING NEW ENTITIES IN MAIN IT ADDS TO THE LIST. 
			// THEY ARE COPIES OF THE ORIGINAL PLAYER, AND IM ONLY CHANGING THE COPIES. DELETE THIS COMMENT AFTER FIX. 
			//Sorting the geometry data
			ref.sort<GeometryData>([](const GeometryData& a, const GeometryData& b) {
				return a < b;
				});
			//creating and filling a vector of gpu instances with the data from the group
			std::vector<GPUInstance> gpuInstances;
			//std::cout << "groupsize " << ref.size() << std::endl;
			for (auto entity : ref) {
				gpuInstances.push_back(ref.get<GPUInstance>(entity));
			}
			//filling a map with all the GeometryData from the group
			unsigned int iterator = 0;
			std::map<int, GeometryData> geoMap;
			for (auto entity : ref) {
				geoMap[iterator] = ref.get<GeometryData>(entity);
				iterator++;
			}
			//emplacing the gpuinstance vector on the renderers entity
			...
			//vkCmdDrawIndexed(commandBuffer, model.accessors[4].count, 1, 0, 0, 0);
			for (size_t i = 0; i < iterator; i++)
			{
				vkCmdDrawIndexed(commandBuffer, geoMap[i].indexCount, 1, geoMap[i].indexStart, geoMap[i].vertexStart, i);
			}
